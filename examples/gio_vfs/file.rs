...
pub mod imp {
    ...
    use gio::{
        Cancellable, FileAttributeInfoList, FileAttributeType, FileAttributeTypeValue,
        FileCopyFlags, FileCreateFlags, FileEnumerator, FileInfo, FileInputStream, FileMonitor,
        FileMonitorFlags, FileOutputStream, FileQueryInfoFlags, IOErrorEnum,
    };
    use glib::{translate::ToGlibPtr, Error, Properties};

    use crate::{
        file_enumerator::MyFileEnumerator,
        file_monitor::MyFileMonitor,
        {update_file_info, SCHEME},
    };
    ...
    impl FileImpl for MyFile {
        ...
        fn resolve_relative_path(&self, relative_path: &str) -> File {
            ...
            let relative_path_as_pb = PathBuf::from(relative_path);
        ...
        fn set_attribute(
            ...
            self.local_file()
                .set_attribute(attribute, type_, value, flags, cancellable)
        }
