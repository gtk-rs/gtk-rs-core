...
use glib::{g_debug, translate::FromGlibPtrBorrow, types::StaticType};
...
pub extern "C" fn g_io_module_load(module_ptr: *mut gio::ffi::GIOModule) {
    let type_module = unsafe { glib::TypeModule::from_glib_borrow(module_ptr as *mut _) };
    let res = register(&type_module);
...
pub extern "C" fn g_io_module_unload(module_ptr: *mut gio::ffi::GIOModule) {
    let type_module = unsafe { glib::TypeModule::from_glib_borrow(module_ptr as *mut _) };
    let res = unregister(&type_module);
    debug_assert!(res.is_ok(), "{}", res.err().unwrap());
}

pub fn register(type_module: &glib::TypeModule) -> Result<(), glib::Error> {
    // register module types
    if !vfs::imp::MyVfs::on_implementation_load(type_module) {
        Err(glib::Error::new(
            IOErrorEnum::Failed,
            "failed to register module type MyVfs",
        ))?;
    }
    if !file::imp::MyFile::on_implementation_load(type_module) {
        Err(glib::Error::new(
            IOErrorEnum::Failed,
            "failed to register module type MyFile",
        ))?;
    }
    if !file_enumerator::imp::MyFileEnumerator::on_implementation_load(type_module) {
        Err(glib::Error::new(
            IOErrorEnum::Failed,
            "failed to register module type MyFileEnumerator",
        ))?;
    }
    if !file_monitor::imp::MyFileMonitor::on_implementation_load(type_module) {
        Err(glib::Error::new(
            IOErrorEnum::Failed,
...
pub fn unregister(type_module: &glib::TypeModule) -> Result<(), glib::Error> {
    // unregister module types
    if !file_monitor::imp::MyFileMonitor::on_implementation_unload(type_module) {
        Err(glib::Error::new(
            IOErrorEnum::Failed,
            "failed to unregister module type MyFileMonitor",
        ))?;
    }
    if !file_enumerator::imp::MyFileEnumerator::on_implementation_unload(type_module) {
        Err(glib::Error::new(
            IOErrorEnum::Failed,
            "failed to register module type MyFileEnumerator",
        ))?;
    }
    if !file::imp::MyFile::on_implementation_unload(type_module) {
        Err(glib::Error::new(
            IOErrorEnum::Failed,
            "failed to register module type MyFile",
        ))?;
    }
    if !vfs::imp::MyVfs::on_implementation_unload(type_module) {
        Err(glib::Error::new(
            IOErrorEnum::Failed,
            "failed to register module type MyVfs",
        ))?;
