...
use gio::{
    prelude::*, subclass::prelude::*, Cancellable, DriveStartFlags, File, FileAttributeType,
    FileCopyFlags, FileCreateFlags, FileMeasureFlags, FileMonitor, FileMonitorEvent,
    FileMonitorFlags, FileQueryInfoFlags, IOErrorEnum, MountMountFlags, MountOperation,
    MountUnmountFlags,
};
...
fn file_set_attribute() {
    ...
    #[cfg(unix)]
    {
        ...
        let res = my_local_file.set_attribute(
            "xattr::my_string2",
            FileAttributeType::String,
            ToGlibPtr::<*mut libc::c_char>::to_glib_none("value2")
                .0
                .into(),
            FileQueryInfoFlags::NONE,
            Cancellable::NONE,
    ...
    // invoke `LocalFile` implementation of `gio::ffi::GFileIface::query_info`
    let res = File::for_path(&my_temp_file.path).query_info(
        "time::access,time::modified,time::access-nsec,xattr::my_string1,xattr::my_string2",
    ...
    #[cfg(unix)]
    {
        let my_string1 = file_info.attribute_string("xattr::my_string1");
        let my_string2 = file_info.attribute_string("xattr::my_string2");
        assert_eq!(
            my_string1.as_ref().map(glib::GString::as_str),
            Some("value1")
        );
        assert_eq!(
            my_string2.as_ref().map(glib::GString::as_str),
            Some("value2")
        );
    }
