                ...
                .map_or((None, std::ptr::null_mut()), |progress_callback| {
                    unsafe extern "C" fn progress_callback_trampoline(
                        ...
                        let progress_callback: &mut Box<dyn FnMut(bool, u64, u64, u64) + 'static> =
                            &mut *(user_data as *mut _);
                        progress_callback(from_glib(reporting), current_size, num_dirs, num_files)
                    }
                    (
                        Some(progress_callback_trampoline as _),
                        progress_callback as *mut Box<dyn FnMut(bool, u64, u64, u64) + 'static>
                            as *mut _,
                    )
                });
...
unsafe extern "C" fn file_copy<T: FileImpl>(
    ...
    let progress_callback = progress_callback.map(|callback| {
        let boxed: Box<dyn FnMut(i64, i64) + 'static> =
            Box::new(move |current_num_bytes, total_num_bytes| unsafe {
...
unsafe extern "C" fn file_mount_mountable<T: FileImpl>(
    ...
    let callback = callback.map(|callback| {
        move |source: &T::Type, res: &AsyncResult| unsafe {
            callback(
...
unsafe extern "C" fn file_measure_disk_usage<T: FileImpl>(
    ...
    let cancellable = Option::<Cancellable>::from_glib_none(cancellable);
    let progress_callback = progress_callback.map(|callback| {
        let boxed: Box<dyn FnMut(bool, u64, u64, u64) + 'static> = Box::new(
