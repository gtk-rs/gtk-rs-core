...
pub trait FileEnumeratorImpl: ObjectImpl + ObjectSubclass<Type: IsA<FileEnumerator>> {
    ...
    ///
    /// NOTE: when the object is dropped, it is finalized, and in that case it's unsafe to access it.
    /// Unfortunately if the enumerator has not been explicitly closed before, GIO automatically calls this vfunc which is unsafe in that case and could fail or could cause memory safety issues.
    ///
    /// In order to close safely the enumerator at drop time, types must also implement [`ObjectImpl::dispose`] and call [`FileEnumeratorExt::close`] (it is safe to access the object within `dispose`):
    /// ```ignore
    ...
    ///     fn dispose(&self) {
    ///         // close the enumerator within `dispose` is safe and make `finalize` to not close it (which would be unsafe).
