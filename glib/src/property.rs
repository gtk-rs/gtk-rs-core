...
impl<T: Property + Default> PropertyWrite for once_cell::sync::OnceCell<T> {
    type Value = T;
    fn set<F: FnOnce(&mut Self::Value)>(&self, f: F) {
...
impl<T: Property + Default> PropertyWrite for once_cell::unsync::OnceCell<T> {
    type Value = T;
    fn set<F: FnOnce(&mut Self::Value)>(&self, f: F) {
        let mut v = Self::Value::default();
        f(&mut v);
...
macro_rules! impl_atomic {
    ($atomic:ty, $valuety:ty) => {
        ...
        impl PropertyWrite for $atomic {
            type Value = $valuety;
            fn set<F: FnOnce(&mut Self::Value)>(&self, f: F) {
                // not ideal, but I have to pass something to `f`...
                let mut tmp = <$valuety as Default>::default();
                f(&mut tmp);
